...
export class UserService {
  ...
  //Récuperer tous les ingénieurs du son//
   ...
   getIngeSons(){
    ...
    return this.firestore.collection<any>('ingeson').snapshotChanges()
      .pipe(
        switchMap(ingesons => {
          const userIds = uniq(ingesons.map(ingeson => ingeson.payload.doc.data()["userprofile_id"])) 
        ...
        map(([ingesons, userProfiles]) => {
          ...
          const id = ingeson.payload.doc.id; 
            return {
              id, ...data,
              userProfile: userProfiles.find(a => a.id === ingeson.payload.doc.data()["userprofile_id"])
            }
          })
        })
      )
 ...
 getRecommendIngeSons(){
  ...
  return this.firestore.collection<any>('ingeson', ref => ref
  .where('recommended', '==', true)).snapshotChanges()
      .pipe(
        switchMap(ingesons => {
          const userIds = uniq(ingesons.map(ingeson => ingeson.payload.doc.data()["userprofile_id"])) 
